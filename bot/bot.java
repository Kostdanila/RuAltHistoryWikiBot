...
{
    MessageChannel refchannel; //канал в котором проводится референдум 
    List<User> voters = new ArrayList<User>();  // список Userов с правом голоса
    List<String> answers = new ArrayList<String>(); // список вариантов ответа
    User Referender; // участник проводящий референдум
	int refestep = 0;
	int repeat = 0; //количество повторений
    String question; // вопрос референдума
    String answerslist = new String(); //сообщение со списком ответов
    HashMap<User, String> results = new HashMap<User, String>(); //ключ - пользователь, знаечние - его ответ
    boolean selfwrite = false; //вписывается ли значение пользователем
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        else if (author == Referender && channel == refchannel && refestep == 1)
        {
        	question = msg;
        	channel.sendMessage("Вопрос: "+question+"\nЗадайте список ответов.").queue();
        	refestep = 2;
        }
        else if (author == Referender && channel == refchannel && refestep == 2)
        {
        ...
        else if (author == Referender && channel == refchannel && refestep == 3)
        {
        	...
        	if (message.getMentionedRoles().isEmpty()==false)
        	{
        	    ...
        	    List<Member> memvoters = guild.getMembersWithRoles(votersRoles); //список memberов с правом голоса
        	...
        	else
        	{
                ...
                channel.sendMessage(text+"Референдум начался.").queue();
                refestep = 4;
        	}
        }
